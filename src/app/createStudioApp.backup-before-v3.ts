...
import { createProgrammaticShotController } from "./studioProgrammaticShotController";
...
import type { RecordingState } from "./studioRecording";
...
export async function createStudioApp({ root }: { root: HTMLDivElement }) {
  ...
  const viewportToolbar = createViewportToolbar({
    ...
    setGridVisible: (visible) => {
      ...
      productionPanel?.setStatus(visible ? "Grid Visible" : "Grid Hidden");
    },
  });
  ...
  window.addEventListener("keydown", (event) => {
    ...
    if (event.key.toLowerCase() === "v") {
      ...
      productionPanel?.setStatus("Safe Area Toggled");
    }
  ...
  transformEditor.controls.addEventListener("objectChange", () => {
    ...
    isPushingEditorTransformToTheatre = true;
    ...
    try {
      ...
      isPushingEditorTransformToTheatre = false;
    }
  });
  ...
  function registerTheatreObject(node: SceneNode) {
    ...
    const props = {
      position: vectorProps(node.root.position, [-10000, 10000]),
      rotation: vectorProps(node.root.rotation, [-Math.PI * 8, Math.PI * 8]),
      scale: vectorProps(node.root.scale, [0.001, 2000]),
    ...
    const unsubscribe = theatreObject.onValuesChange((values) => {
      if (!isTheatreSequencePlaying || isPushingEditorTransformToTheatre) {
        return;
      }
  ...
  function registerTheatreMainCamera() {
    ...
    theatreMainCamera = theatreSheet.object(
      ...
      {
        position: vectorProps(camera.position, [-50000, 50000]),
        rotation: vectorProps(camera.rotation, [-Math.PI * 8, Math.PI * 8]),
        camera: types.compound({
  ...
  function getCameraShotAnimations() {
    return timelineController.getCameraShotAnimations();
  }

  function getCameraShotTimelineItems(): TimelineDockItem[] {
    return timelineController.getCameraShotTimelineItems();
  }
      ...
      .forEach((button) => {
        button.onclick = () => {
          if (button.dataset.theatreShotId) {
            timelineController.selectCameraShot(button.dataset.theatreShotId);
          }
        };
      });
  }
  ...
  function previewCameraShot(shotId: string) {
    ...
    timelineController.setActiveShotId(animation.id);
    timelineController.refresh();
  ...
  function applyObjectMotion(
    ...
    timelineController.addTimelineAnimation(
      createObjectMotionAnimation({
        ...
        delay:
          options.delay ??
          getCameraShotAnimations().find(
            (shot) => shot.id === timelineController.getActiveShotId(),
          )?.delay ??
          0,
...
function cameraShotToShotType(shot: CameraShot): ShotType {
  ...
  function applyCameraShot(
    ...
    const delay = options.delay ?? timelineController.getCameraShotCursor();
    timelineController.setCameraShotCursor(
    ...
    if (newestShot) timelineController.setActiveShotId(newestShot.id);

    timelineController.rebuildCameraShotTiming();
    if (!options.silent) {
  ...
  function playTheatreSequence() {
    ...
    isTheatreSequencePlaying = true;

    void theatreSheet.sequence.play().finally(() => {
      isTheatreSequencePlaying = false;
      controls.enabled = activeRenderCameraId === "main";
    });
  ...
  function bakeShotsToTheatre() {
    ...
    const clips = timelineController
      .getAnimations()
      .filter(
        (animation) =>
    ...
    try {
      ...
      studio.transaction(({ set }) => {
        set(
          ...
          Math.max(timelineController.getTimelineDuration(), 0.5),
        );
      });
  ...
  function duplicateNode(node: SceneNode) {
    ...
    const source = (node.metadata.source as NodeSource) ?? {
      type: "generated",
    };

    const duplicate = attachObject(cloneName, cloneRoot, source);
    ...
    transformEditor.refresh();
    refreshProductionCameras();
    transformEditor.selectNode(duplicate.id);
  ...
  function focusSelectedNode(node: SceneNode | null) {
    if (!node || activeRenderCameraId !== "main") return;
    ...
    const direction = new THREE.Vector3()
      .subVectors(camera.position, controls.getTarget(new THREE.Vector3()))
      .normalize();
  ...
  function serializeScene(): SavedScene {
    return serializeSceneData({
      ...
      nodes: registry.getAll(),
      serializeTimeline: timelineController.serializeTimeline,
    });
  }
  ...
  async function loadSavedScene(savedScene: SavedScene) {
    ...
    hierarchyPanel.refresh();
    transformEditor.clearSelection();
    timelineController.restoreTimeline(savedScene.timeline);
  ...
  const programController = createProgrammaticShotController({
    scene,
    camera,
    setStatus: sceneBuilder.setStatus,
    getSceneNodes: () => registry.getAll(),
  });
  timelineController = createTimelineController({
    setStatus: sceneBuilder.setStatus,
    refreshProductionShots: (shots) => productionPanel?.refreshShots(shots),
    renderTheatreShotPane,
    getTimelineDock: () => timelineDock,
    applyCameraShot,
    applyObjectMotion,
    findSceneNodeByName,
  });
  function playDirectorTimeline() {
    timelineController.playTimeline();
  }

  function pauseDirectorTimeline() {
    timelineController.pauseTimeline();
    shotOverlayScheduler.stop();
  }

  function stopDirectorTimeline() {
  ...
  productionPanel = createProductionPanel({
    ...
    pauseTimeline: pauseDirectorTimeline,
    ...
    viewSelectedCamera,
    viewMainCamera,
    removeCameraShot: timelineController.removeCameraShot,
    moveCameraShot: timelineController.moveCameraShot,
    selectCameraShot: timelineController.selectCameraShot,
    duplicateCameraShot: timelineController.duplicateCameraShot,
    previewCameraShot,
  ...
  function startRecording(
    ...
    overlayManager.setAspect(toOverlayAspect(aspect));
    overlayManager.showSafeArea(true);
  ...
  function recordTimeline(
    ...
    const timelineDuration = timelineController.getTimelineDuration();
    const safeDuration = Math.max(timelineDuration + 2, seconds, 1);

    timelineController.setStopRecordingWhenTimelineEnds(true);
  ...
  function resize() {
    ...
    overlayManager.controller.setViewport(width, height);
  }
  ...
  function animate() {
    ...
    if (timelineController.update(delta)) {
      timelineController.markRecordingStopHandled();
      recordingManager.stop();
    }
